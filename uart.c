...
void putch(unsigned char byte) 
{
	...
	while(!TXIF)         //TXIF = 0 REGISTER IS FULL , 1 REGISTER IS EMPTY
	{
		continue;
	}
	TXIF = 0;
...
int puts(const char *s)
{
	while(*s)		
	{
		putch(*s++);	// WE WILL LOAD BYTE BY BYTE
	}
	return 0;
}
...
unsigned char getch(void)
{
	...
	while(!RCIF)          //RCIF = 0 , REGISTER EMPTY , 1, REGISTER FULL
	{
		continue;
	}
	RCIF = 0;
...
unsigned char getche(void)       //RECEIVE AND TRANSMIT THE SAME DATA
{
	unsigned char c;
